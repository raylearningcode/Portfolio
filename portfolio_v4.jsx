        ...
        <div style={{minWidth:0}}>
          ...
          <div style={{...f2,marginBottom:"1.6rem"}}>
            <p style={{fontFamily:"Inter,sans-serif",fontSize:"clamp(0.92rem,1.9vw,1.22rem)",color:T.textMid,margin:0,lineHeight:1.65}}>
              CS Student &amp; Security Engineer in Progress<br/>
              <span style={{fontSize:"0.87em",color:T.textDim}}>Indonesia 🇮🇩 &nbsp;→&nbsp; Taiwan 🇹🇼 &nbsp;→&nbsp; Germany 🇩🇪</span>
            </p>
          </div>
          ...
          <div style={{...f4,display:"flex",gap:"0.75rem",flexWrap:"wrap"}}>
            {[{l:"VIEW MY WORK →",cb:()=>onNav("LABS"),primary:true},{l:"GET IN TOUCH",cb:()=>onNav("CONTACT"),primary:false}].map(b=>(
              <button key={b.l} onClick={b.cb} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.63rem",letterSpacing:"0.14em",padding:"0.72rem 1.5rem",borderRadius:"4px",cursor:"pointer",transition:"all 0.2s",background:b.primary?`${T.accent}12`:"transparent",border:`1px solid ${b.primary?T.accent:T.border}`,color:b.primary?T.accent:T.textMid,boxShadow:b.primary?`0 0 20px ${T.accent}18`:"none"}}
                onMouseEnter={e=>{e.currentTarget.style.background=b.primary?`${T.accent}22`:`${T.border}`;e.currentTarget.style.boxShadow=b.primary?`0 0 32px ${T.accent}28`:"none";}}
                onMouseLeave={e=>{e.currentTarget.style.background=b.primary?`${T.accent}12`:"transparent";e.currentTarget.style.boxShadow=b.primary?`0 0 20px ${T.accent}18`:"none";}}>
                {b.l}
              </button>
            ))}
...
const TL=[
  ...
  {year:"2026+",icon:"🇩🇪",title:"M.Sc. Cybersecurity, Germany",detail:"TU Berlin / KIT / RWTH. Studying German daily."},
];
const About=({T})=>{
  const[ref,fade]=useScrollFade();
  return(
        ...
        <div className="about-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3.5rem",alignItems:"start"}}>
          <div>
            ...
            <p style={{fontFamily:"Inter,sans-serif",fontSize:"0.88rem",color:T.textMid,lineHeight:1.9,marginBottom:"1rem"}}>
              I'm <strong style={{color:T.text}}>Rafi</strong> — a CS student in Taiwan, originally from Indonesia. I'm obsessed with how systems break and how to build them so they don't. My focus: <strong style={{color:T.accent}}>cybersecurity, full-stack development, and automation</strong>.
            </p>
            <p style={{fontFamily:"Inter,sans-serif",fontSize:"0.88rem",color:T.textMid,lineHeight:1.9,marginBottom:"1.8rem"}}>
              Outside of hacking and shipping code, I'm learning German and Mandarin, competing in CTFs, and building toward a Master's in Cybersecurity in Germany.
            </p>
            ...
            <div style={{padding:"1rem 1.2rem",background:`${T.green}07`,border:`1px solid ${T.green}20`,borderRadius:"8px"}}>
              ...
              {["📚 Studying: eJPT cert + German (A2 → B1)","🛠️ Building: CTF vault + Python security tools","🎯 Goal: M.Sc. Cybersecurity, Germany 2026"].map(l=>(
                <div key={l} style={{fontFamily:"Inter,sans-serif",fontSize:"0.78rem",color:T.textMid,lineHeight:2}}>{l}</div>
              ))}
            </div>
          </div>
...
// ─── ARSENAL ──────────────────────────────────────────────────────────────────
const SK=[
  {cat:"SECURITY",         c:"#FF0055",items:[["Web Exploitation",82],["Network Security",70],["CTF / Forensics",75],["OSINT & Recon",68],["Linux / CLI",88]]},
  {cat:"PYTHON & SCRIPTING",c:"#00FF41",items:[["Python 3",85],["Automation & Bots",80],["Scapy / Sockets",72],["SQLite / DB",68],["Bash Scripting",74]]},
  {cat:"WEB & DESIGN",     c:"#00F0FF",items:[["HTML / CSS",90],["React / Next.js",78],["Tailwind CSS",83],["UI/UX & Figma",68],["Responsive Design",85]]},
  {cat:"LANGUAGES",        c:"#FFD700",items:[["Indonesian 🇮🇩",100],["English",92],["Mandarin 🇹🇼",65],["German 🇩🇪",38]]},
];
const Bar=({name,val,color,T})=>{
  const[on,setOn]=useState(false);const ref=useRef();
  useEffect(()=>{const obs=new IntersectionObserver(([e])=>{if(e.isIntersecting)setOn(true);},{threshold:0.1});if(ref.current)obs.observe(ref.current);return()=>obs.disconnect();},[]);
  return(
    <div ref={ref} style={{marginBottom:"0.88rem"}}
      onMouseEnter={e=>e.currentTarget.querySelector(".bar-fill").style.opacity="1"}
      onMouseLeave={e=>e.currentTarget.querySelector(".bar-fill").style.opacity="0.8"}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:"0.2rem"}}>
        <span style={{fontFamily:"'Fira Code',monospace",fontSize:"0.7rem",color:T.textMid}}>{name}</span>
        <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.6rem",color}}>{val}%</span>
      </div>
      <div style={{height:"2px",background:T.border,borderRadius:"1px",overflow:"hidden"}}>
        <div className="bar-fill" style={{height:"100%",borderRadius:"1px",background:color,opacity:0.8,width:on?`${val}%`:"0%",transition:"width 1.1s cubic-bezier(0.4,0,0.2,1),opacity 0.2s",boxShadow:`0 0 6px ${color}50`}}/>
      </div>
    </div>
  );
};
const Arsenal=({T})=>{
  ...
  return(
    <Sec id="ARSENAL" T={T}>
      ...
      <div ref={ref} style={{...fade,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:"1.1rem"}}>
        {SK.map(g=>(
          <div key={g.cat} style={{background:T.surface,border:`1px solid ${g.c}18`,borderRadius:"10px",padding:"1.35rem",backdropFilter:"blur(10px)",transition:"box-shadow 0.22s,transform 0.22s",cursor:"default"}}
            ...
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.57rem",color:g.c,letterSpacing:"0.18em",marginBottom:"1.1rem",paddingBottom:"0.55rem",borderBottom:`1px solid ${g.c}20`}}>[{g.cat}]</div>
            {g.items.map(([n,v])=><Bar key={n} name={n} val={v} color={g.c} T={T}/>)}
          </div>
        ))}
      </div>
    </Sec>
...
const PROJS=[
  ...
  {id:"02",cat:"SECURITY", n:"SQLi Scanner",     tl:"Automated SQL Injection Detector",   desc:"Fires 40+ payload variants across GET/POST params, detects error-based and blind SQLi, generates a severity report per endpoint.",                     tags:["Python","Web Security","OWASP"],             st:"COMPLETE", sc:"#00F0FF",gh:"#"},
  ...
  {id:"08",cat:"FULLSTACK",n:"Malware Sandbox",  tl:"Dynamic Malware Analysis Lab",       desc:"Isolated VM using Cuckoo Sandbox for behavioral malware analysis. Logs syscalls, network calls, file mutations. 15+ documented samples.",               tags:["Cuckoo","Linux","VM","Python"],              st:"LAB",      sc:"#FF0055",gh:"#"},
];
const Card=({p,T,idx})=>{
  ...
  return(
    <div ref={ref} style={{...fade,transitionDelay:`${idx*55}ms`}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}>
      <div style={{background:hov?T.surfaceHov:T.surface,border:`1px solid ${hov?cc+"45":T.border}`,borderRadius:"10px",padding:"1.35rem",transition:"all 0.25s ease",transform:hov?"translateY(-5px)":"none",boxShadow:hov?`0 14px 40px ${cc}12,${T.shadow}`:"none",display:"flex",flexDirection:"column",backdropFilter:"blur(10px)",height:"100%"}}>
        ...
        <div style={{fontFamily:"Inter,sans-serif",fontSize:"0.78rem",color:T.textMid,lineHeight:1.78,marginBottom:"1rem",flex:1}}>{p.desc}</div>
      ...
      <div ref={ref} style={{...fade,border:`1px solid ${T.border}`,borderRadius:"10px",overflow:"hidden",backdropFilter:"blur(10px)"}}>
        {WUS.map((w,i)=>(
          <div key={i} className="wu-row" style={{display:"grid",gridTemplateColumns:"78px 66px 1fr 54px",alignItems:"center",gap:"1rem",padding:"0.88rem 1.2rem",background:T.surface,borderBottom:i<WUS.length-1?`1px solid ${T.border}`:"none",cursor:"pointer",transition:"background 0.15s,transform 0.15s"}}
            ...
            <div>
              <div style={{fontFamily:"Inter,sans-serif",fontSize:"0.82rem",color:T.text,fontWeight:500,marginBottom:"0.18rem"}}>{w.title}</div>
...
const Contact=({T})=>{
  ...
  const submit=()=>{if(!form.name||!form.email||!form.msg){setErr("All fields required.");return;}setSent(true);setErr("");};
  const inp={width:"100%",boxSizing:"border-box",background:T.inputBg,border:`1px solid ${T.border}`,borderRadius:"5px",padding:"0.65rem 0.9rem",color:T.text,outline:"none",fontFamily:"'Fira Code',monospace",fontSize:"0.78rem",transition:"border-color 0.2s"};
  const[ref,fade]=useScrollFade();
  return(
    <Sec id="CONTACT" T={T}>
      ...
      <div ref={ref} style={{...fade}}>
        <div className="contact-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3.5rem",alignItems:"start"}}>
          <div>
            ...
            <p style={{fontFamily:"Inter,sans-serif",fontSize:"0.84rem",color:T.textMid,lineHeight:1.85,marginBottom:"2rem"}}>
          ...
          <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"1.6rem",backdropFilter:"blur(12px)"}}>
            {sent?(
              <div style={{textAlign:"center",padding:"2.5rem 1rem"}}>
                <div style={{fontSize:"2.5rem",marginBottom:"0.8rem"}}>✓</div>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.78rem",color:T.green}}>MESSAGE SENT</div>
                <div style={{fontFamily:"Inter,sans-serif",fontSize:"0.74rem",color:T.textDim,marginTop:"0.5rem"}}>I'll be in touch soon.</div>
              </div>
            ):(
              <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.57rem",color:T.green,letterSpacing:"0.15em"}}>// NEW_MESSAGE.sh</div>
                {[{k:"name",l:"NAME",p:"Your name"},{k:"email",l:"EMAIL",p:"your@email.com"}].map(f=>(
                  <div key={f.k}>
                    <label style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.54rem",color:T.textDim,letterSpacing:"0.1em",display:"block",marginBottom:"0.32rem"}}>{f.l}</label>
                    <input value={form[f.k]} onChange={upd(f.k)} placeholder={f.p} style={inp}
                      onFocus={e=>e.target.style.borderColor=T.accent+"80"}
                      onBlur={e=>e.target.style.borderColor=T.border}/>
                  </div>
                ))}
                <div>
                  <label style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.54rem",color:T.textDim,letterSpacing:"0.1em",display:"block",marginBottom:"0.32rem"}}>MESSAGE</label>
                  <textarea value={form.msg} onChange={upd("msg")} rows={4} placeholder="What's on your mind?" style={{...inp,resize:"vertical"}}
                    onFocus={e=>e.target.style.borderColor=T.accent+"80"}
                    onBlur={e=>e.target.style.borderColor=T.border}/>
                </div>
                {err&&<div style={{fontFamily:"'Fira Code',monospace",fontSize:"0.65rem",color:T.red}}>{err}</div>}
                <button onClick={submit} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"0.62rem",letterSpacing:"0.18em",padding:"0.75rem",background:`${T.accent}10`,border:`1px solid ${T.accent}`,color:T.accent,cursor:"pointer",borderRadius:"5px",transition:"all 0.2s"}}
                  onMouseEnter={e=>{e.currentTarget.style.background=`${T.accent}1e`;e.currentTarget.style.boxShadow=`0 0 22px ${T.accent}25`;}}
